...
/**
 * Hides the editor on certain pages.
 */
...
function hide_editor() {
	...
	$filter = [
		['type'=>'page', 'template' => 'page-templates/page-people.php' ],
		['type'=>'page', 'template' => 'page-templates/page-projects.php' ],
		['type'=>'page', 'template' => 'page-templates/page-events.php' ],
		['type'=>'page', 'template' => 'page-templates/page-front.php' ],
		['type'=>'page', 'template' => 'page-templates/page-blog.php' ],
		['type'=>'page', 'template' => 'page-templates/page-about.php' ],
	];
	...
	else if(isset($_POST['post_ID'])) $post_id = intval($_POST['post_ID'] );

	//if post_id is 0 we have a new post, then curPostType will be set.

	$curPostType = isset($_GET['post_type']) ? $_GET['post_type'] : '';
	...
	foreach($filter as $f){
		...
		if($f['type'] == get_post_type($post_id) || $f['type'] == $curPostType){
			if(isset($f['template'])){
				//if required check template.
				$remove = ($f['template'] == get_post_meta($post_id, '_wp_page_template', true));
			}else{
				$remove = true;
			}
		}
		...
		if($remove)
			remove_post_type_support($f['type'], 'editor');
